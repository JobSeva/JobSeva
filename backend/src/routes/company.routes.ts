...
const profileFieldsSchema = {
  name: z.string().min(2).max(120).regex(/^[a-zA-Z0-9\s.,!?'"()\/&+-]+$/, "Name contains invalid characters"),
  logo: z.string().min(1).max(12),
  tagline: z.string().max(180).regex(/^[a-zA-Z0-9\s.,!?'"()\/&+-]+$/, "Tagline contains invalid characters"),
  about: z.string().max(5000),
  industry: z.string().max(120).regex(/^[a-zA-Z0-9\s.,!?'"()\/&+-]+$/, "Industry contains invalid characters"),
  size: z.string().max(60),
  founded: z.number().int().min(1800).max(new Date().getFullYear()),
  headquarters: z.string().max(120).regex(/^[a-zA-Z0-9\s.,!?'"()\/&+-]+$/, "Headquarters contains invalid characters"),
  ...
  recruiterName: z.string().min(2).max(120).regex(/^[a-zA-Z0-9\s.,!?'"()\/&+-]+$/, "Recruiter name contains invalid characters"),
  recruiterDesignation: z.string().max(120).regex(/^[a-zA-Z0-9\s.,!?'"()\/&+-]+$/, "Recruiter designation contains invalid characters"),
  isHiring: z.boolean(),
